...
>
> 平台范围说明：本仓库当前**仅支持 B站 / 抖音**（直播监控 + 抖音新作品检测）。**小红书直播监控已于 2026-07-10 从代码中移除，当前未支持**；下文涉及小红书的仅为外部项目调研与通用技术路线参考。
...
| **本仓库** | 多平台**直播/新作推送**（B站 / 抖音） | ✅ 官方 API | ✅ 服务端 HTML 多策略 | ❌ 当前未支持 | 聚焦 B站 / 抖音 |
...
### 1.3 小红书（外部项目视角：动态✅ / 开播❌）
- 以 `aio-dynamic-push` 为例：其 `query_xhs.py` **只有 `query_dynamic`，`query()` 里没有直播分支**（README 表格也标开播❌）。
...
> 小结：连最像本仓库的纯推送项目 `aio-dynamic-push` 都只做小红书动态、不做开播检测。本仓库曾尝试用无头 Chromium 渲染直播间（`xgplayer-is-live` / `xhsplayer-skin-live`）补这块，但因短链易变、数据中心 IP 风控等维护成本过高，已于 2026-07-10 移除，**当前未支持**。
...
- 小红书（外部项目能力）：支持**作者主页地址**录制，但「每次开播都要重新获取一次链接」（直播链接不固定），靠短链 `xhslink.com` 解析 + 网页/接口提取。
...
| **小红书**（参考路线，本仓库未实现） | ① 动态：`__INITIAL_STATE__`（能拿到）；② **开播：无头浏览器渲染直播间 + 播放器 class** | `xgplayer-is-live` / `xhsplayer-skin-live` | SSR 空模板，签名 API 不回写 script，纯 HTTP 拿不到开播态；数据中心 IP 触发风控页 |
...
2. **B站**：当前方案已与业界一致，无需改。
3. **小红书开播**：本仓库已移除该能力（维护成本过高），当前未支持；如未来重做需重新评估短链易变与风控成本。
4. **轮询节奏**：录制类靠"长间隔 + 宁漏勿错"，本仓库纯推送靠"短间隔 + 去重账本防抖"，路线不同但都正确。

> 所有参考项目均为"检测即触发后续动作（推送/录制）"的轮询模型，与本仓库架构同源。小红书开播检测是业界纯推送项目的共同盲区，本仓库当前也未支持。
