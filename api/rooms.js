...
export const RoomValidator = {
  ...
  validateInput(body) {
    ...
    const name = (rawName != null && String(rawName).trim() !== "")
      ? String(rawName).trim()
      : id;
    const room = { platform: platform, id: id, name: name };
    // 透传可选 sec_uid：调用方若已解析出 sec_uid（如用户直接粘贴），一并保留，
    // 使该账号等价于「预存 sec_uid」的账号，立即生效。
    if (body.sec_uid != null && String(body.sec_uid).trim() !== "") {
      room.sec_uid = String(body.sec_uid).trim();
    }
