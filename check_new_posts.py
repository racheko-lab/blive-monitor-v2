...
def get_latest_aweme(context, sec_uid: str) -> Optional[Dict[str, Any]]:
    """两层策略获取用户最新作品，返回标准化 dict（含 _conf 置信度）或 None。

    策略顺序：
      1) 拦截页面自身签名的 aweme/post 响应（需登录 Cookie）：返回真实作品列表，
         含 create_time / desc / nickname，置信度 "api"，可精确推送「新作品」；
      2) 退化（无需 Cookie）：解析 user/profile/other 的 aweme_count，
         作品数增加时推测「可能有新作品」，置信度 "count"，仅作提示、链接到主页。
    """
    page = context.new_page()
    ...
    def on_resp(resp):
        ...
        try:
            if "/aweme/v1/web/aweme/post/" in u:
                captured["post"] = resp.body().decode("utf-8", "replace")
            elif "user/profile/other" in u:
                captured["profile"] = resp.body().decode("utf-8", "replace")
        except Exception:
            pass
    ...
    try:
        ...
        # 策略 1：API 响应（需登录 Cookie 才会返回真实列表）
        items = parse_aweme_list(captured.get("post")) if captured.get("post") else []
        if items:
        ...
        count = parse_aweme_count(captured.get("profile"))
